...
import { BSON, FindOptions } from 'mongodb';
import { env } from '@core/config/envConfig';
import * as crypto from 'crypto';

let valid = ""
class _dbUtils {
    private static intance: _dbUtils;

    static get instance(): _dbUtils {
        return this.intance || (this.intance = new this());
    }


    objectToStringParser2 = {

        stringify: function (value, context = {}) {  // ← Error: tenías un paréntesis mal colocado
    ...
    convertBSON(_obect: any = {}) {
        ...
        if (typeof _obect === 'string' && _obect.match(/^[0-9a-fA-F]{24}$/)) {
            return new BSON.ObjectId(_obect);
        } else if (
            Array.isArray(_obect) &&
            !(_obect instanceof BSON.ObjectId)
        ) {
            const _list = [];
        ...
        for (var property in _obect) {
            ...
            if (
                _obect[property] &&
                !(_obect[property] instanceof BSON.ObjectId) &&
                ...
                isVal = true;
                _filter['_id'] = new BSON.ObjectId(_obect[property]);
            } else if (
                _obect[property] &&
                typeof _obect[property] === 'string' &&
                _obect[property].match(/^[0-9a-fA-F]{24}$/)
                ...
                isVal = true;
                _filter[property] = new BSON.ObjectId(_obect[property]);
    ...
    captureOptions(options?: FindOptions) {
        let _object = {};

        if (!options) return _object;
        ...
        if (options.projection) {
            _object['projection'] = this.objectToStringParser.parse(
                options.projection
            );
        }

        const _option: any = options;
        if (_option.project) {
            _object['projection'] = this.objectToStringParser.parse(
                _option.project
            );
    ...
    convertJSON = (_data, _keys = "inicio", _funciont = "inicio") => {
        var data = _data;
        ...
        for (var property in data) {
            ...
            if (_item && _item instanceof BSON.ObjectId) {
    ...
    _generateJWTToken = (user): string => {
        ...
        const _appId = env.AWS_ACCESS_APP_ID;
        const _secret = env.JWT_SECRET;
