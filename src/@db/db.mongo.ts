...
export class DbMongo {
  ...
  private _connect: MongoClient;
  private activeWatchers = new Map<string, Set<any>>();
  ...
  private cleanDeadWatchers() {
    for (const [key, watchers] of this.activeWatchers) {
      ...
      if (watchers.size === 0) this.activeWatchers.delete(key);
    }
  }
   ...
   */
  private notifyChange(change: any) {
  ...
  private matchesFilter(change: any, filter: any): boolean {
    ...
    for (const [key, expected] of Object.entries(filter)) {
      if (key === 'operationType' || key === 'fullDocument') continue;
   ...
   */
  async watch(database: string, collection: string, args: any = {}, res: any) {
   ...
   */
  async executeOperation(operation: string, database: string, collection: string, ...args: any[]) {
    const client = await this.connect;
    const db = client.db(database);
    const coll :any= db.collection(collection);
    ...
    try {
      ...
      const shouldNotify =
        (operation === 'insertOne' && result?.acknowledged) ||
  ...
  async insertOne(database: string, collection: string, document: any) {
    return this.executeOperation('insertOne', database, collection, document);
  }

  async insertMany(database: string, collection: string, documents: any[]) {
    return this.executeOperation('insertMany', database, collection, documents);
  }

  async updateOne(database: string, collection: string, filter: any, update: any, options: any = {}) {
    return this.executeOperation('updateOne', database, collection, filter, update, options);
  }

  async updateMany(database: string, collection: string, filter: any, update: any, options: any = {}) {
    return this.executeOperation('updateMany', database, collection, filter, update, options);
  }

  async deleteOne(database: string, collection: string, filter: any) {
    return this.executeOperation('deleteOne', database, collection, filter);
  }

  async deleteMany(database: string, collection: string, filter: any) {
    return this.executeOperation('deleteMany', database, collection, filter);
  }

  async replaceOne(database: string, collection: string, filter: any, replacement: any, options: any = {}) {
    return this.executeOperation('replaceOne', database, collection, filter, replacement, options);
  }

  async findOneAndUpdate(database: string, collection: string, filter: any, update: any, options: any = {}) {
    return this.executeOperation('findOneAndUpdate', database, collection, filter, update, options);
  }

  async find(database: string, collection: string, filter: any = {}, options: any = {}) {
    const client = await this.connect;
    return client.db(database).collection(collection).find(filter, options).toArray();
  }

  async findOne(database: string, collection: string, filter: any = {}, options: any = {}) {
    const client = await this.connect;
    return client.db(database).collection(collection).findOne(filter, options);
  }

  async findById(database: string, collection: string, id: string) {
    const client = await this.connect;
    return client.db(database).collection(collection).findOne({ _id: new ObjectId(id) });
  }

  async aggregate(database: string, collection: string, pipeline: any[]) {
    const client = await this.connect;
    return client.db(database).collection(collection).aggregate(pipeline).toArray();
  }

  async count(database: string, collection: string, filter: any = {}) {
    const client = await this.connect;
