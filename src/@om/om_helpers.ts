...
export function guardarJSONCompleto(objeto, rutaArchivo) {
...
import { Route } from "@om/route/route.types";
...
export interface INeewHierarchy {
    component: any;
    path: string;
    level: number;
    selector?: string;
...
export interface IDiscardHierarchy {
    component: any;
    path: string;
    level: number;
}
...
export interface HierarchyAnalysis {
    discardedComponents: IDiscardHierarchy[];
    newHierarchy: INeewHierarchy[];
    commonRoot?: {
        component: any;
        path: string;
    };
...
// ────────────────────────────────────────────────
// 1. Actualizamos la interfaz (agregamos resolve opcional)
// ────────────────────────────────────────────────
export interface INeewHierarchy {
    component: any;
    path: string;
    ...
    selector?: string;
    isRoot: boolean;
    resolve?: any;           // ← agregado (puede ser el objeto resolve de la ruta)
}

// ────────────────────────────────────────────────
// 2. La función analyzeRouteHierarchy con el cambio mínimo
// ────────────────────────────────────────────────
export function analyzeRouteHierarchy(
    routes: Route[],
    oldPath: string,
    newPath: string
): HierarchyAnalysis {
...
function getFullHierarchy(routes: Route[], pathSegments: string[]): Array<{
    component: any;
    fullPath: string;
    segments: string[];
    level: number;
...
function findRouteBySegment(routes: Route[], segment: string): Route | undefined {
    return routes.find(route => route.path === segment);
}

function findDiscardedComponents(
    oldHierarchy: ReturnType<typeof getFullHierarchy>,
    newHierarchy: ReturnType<typeof getFullHierarchy>
): HierarchyAnalysis['discardedComponents'] {
    const discarded: HierarchyAnalysis['discardedComponents'] = [];
...
function hasCommonRoot(oldHierarchy: any[], newHierarchy: any[]): boolean {
...
function findDivergenceIndex(oldHierarchy: any[], newHierarchy: any[]): number {
    let i = 0;
    while (
        i < oldHierarchy.length &&
...
function findCommonRoot(oldHierarchy: any[], newHierarchy: any[]): HierarchyAnalysis['commonRoot'] {
    if (oldHierarchy.length === 0 || newHierarchy.length === 0) {
        return undefined;
    }

    const firstCommon = oldHierarchy.find((oldItem, index) =>
        index < newHierarchy.length &&
...
export function analyzeRouteHierarchySimple(
    routes: Route[],
    oldPath: string,
    newPath: string
) {
    ...
    const getComponents = (segments: string[]) => {
        const components: Array<{ component: any, path: string }> = [];
        let current = routes;
    ...
    let divergeIndex = 0;
    while (
        divergeIndex < oldComponents.length &&
...
export function resolveRouteObservablesSync(routeItem) {
